...
def redact_pii(text, detections):
    ...
    try:
        header_pat = re.compile(r'(?im)^\s*(Education|Experience|Projects|Work\s*Experience|Professional\s*Experience)\s*:?\s*$', re.IGNORECASE)
        lines = text.splitlines()
        ...
        for idx, line in enumerate(lines):
            ...
            if not m:
                continue
            hdr = m.group(1).title()
            if hdr in ("Work Experience", "Professional Experience"):
                hdr = "Experience"
            norm_hdr = f"{hdr}:"
            # Find the first non-empty content line after the header to anchor insertion
            anchor = ""
            for j in range(idx + 1, min(idx + 6, len(lines))):  # look ahead a few lines only
                nxt = lines[j].strip()
                if nxt:
                    anchor = nxt[:120]
                    break
    ...
    try:
        platform_handle_line = (
            ...
            r')?\s*[:|/\\-]*\s*/?[@]?[A-Za-z0-9][A-Za-z0-9_\-]{1,39}\s*$\n?'
        )
